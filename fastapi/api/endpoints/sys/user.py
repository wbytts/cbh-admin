...
async def account_login(req: Request, post: user.AccountLogin):
    ...
    if post.username and post.password:
        ...
        if not get_user.password:
            return fail(msg=f"用户{post.username}密码验证失败!")
        if not utils.password.check_password(post.password, get_user.password):
            return fail(msg=f"用户{post.username}密码验证失败!")
...
@router.put(
    "/modify/mobile", dependencies=[Security(check_permissions)], summary="用户手机号修改"
)
async def update_user_phone(req: Request, post: ModifyMobile):
    """
    修改绑定手机号
    :param req:
    :param post:

...
async def mysql_does_not_exist(_: Request, exc: DoesNotExist) -> JSONResponse:
    ...
    return JSONResponse(
        {"code": -1, "message": "发出的请求针对的是不存在的记录，服务器没有进行操作。", "data": []},
        status_code=404,
    )
...
async def mysql_operational_error(_: Request, exc: OperationalError) -> JSONResponse:
    ...
    return JSONResponse({"code": -1, "message": "数据操作失败", "data": []}, status_code=500)
...
class UnicornException(Exception):
    def __init__(self, code: str, errmsg: str, data: Any = None) -> None:
        """
        失败返回格式
        :param code:
        :param errmsg:
